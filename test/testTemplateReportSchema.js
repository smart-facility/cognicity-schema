...
export default function (db, instance){
  // Cards endpoint
  describe('Template report data source functionality', function() {
    ...
    // Test
    it ('Correctly pushes the report to the cognicity.all_reports table', function(done){
    ...
    // Clean up
    after ('Remove dummy template report data', function(done){

      // Remove dummy report
      let query1 = "DELETE FROM template_data_source.reports WHERE pkey = $1";
      let values1 = [ report_fkey ]
      db.none(query1, values1)
        .then(function(){
          done();
        })
        .catch(function(error){
      ...
      let query2 = "DELETE FROM template_data_source.reports WHERE pkey = $1";
      let values2 = [ report_pkey ]
      db.none(query2, values2)
        .then(function(){
          done();
        })
        .catch(function(error){
