...
export default (db, instance) => {
  ...
  describe('Floodgauge report schema functionality: ' + instance.name, () => {
    ...
    // Insert test data
    let query = `INSERT INTO floodgauge.reports (gaugeID, measureDateTime,
      depth, deviceID, reportType, level, notificationFlag, gaugeNameId,
      gaugeNameEn, gaugeNameJp, warningLevel, warningNameId, warningNameEn,
      warningNameJp, observation_comment, the_geom)
      VALUES ('1', now(), 100, '1', 'confirmed', '100', 0, 'gauge',
      'gauge', 'gauge', 4, 'warningname', 'warningname',
      'warningname', 'comment', ST_GeomFromText('POINT($1 $2)', 4326))
      RETURNING pkey`;
