...
export default (db, instance) => {
  // Cards endpoint
  describe('Detik report schema functionality: ' + instance.name, () => {
    let reportFkey;
    let reportPkey;

    before('Insert dummy Detik data', (done) => {
    // Insert test data
    let query = `INSERT INTO detik.reports (contribution_id, created_at,
      disaster_type, text,  lang, url, image_url, title, the_geom)
      VALUES (9999, now(), 'flood', 'report text', 'en', 'no_url',
      'no_image', 'title', ST_GeomFromText('POINT($1 $2)', 4326))
      RETURNING pkey`;
    ...
    // Test
    it('Correctly pushes to cognicity.all_reports table', (done) => {
      // Check the test data has been assigned correct polygon
      let query = `SELECT * FROM cognicity.all_reports WHERE
                    fkey = $1 AND source = 'detik'`;
